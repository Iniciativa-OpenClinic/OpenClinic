...
2. **Contraste é regra, não gosto.** Piso WCAG AA (4,5:1) em todo texto; leitura contínua (evolução, anamnese, documentação) sempre em tinta escura sobre papel claro, acima de 7:1 (AAA). Todo par cor+fundo desta identidade foi medido — inclusive os nove pares do cabeçalho escuro, todos ≥5,6:1.
...
**Proporção de uso** (aproximada, o "60-30-10" da marca): neutros dominam (~60% — papel, cartões, tinta), o teal estrutura (~30% — moldura, ações, navegação ativa), e o calor pontua (≤10% — terracota em encaixes, avisos de cuidado, um destaque por tela).
...
| Primária clara | `#55A5C2` | Texto/acento primário no escuro (o `#23768A` reprovaria como texto: 3,3:1) | 6,2:1 · AA |
...
- **Superfícies de confiança e decisão** (estados vazios, diálogos de risco — cancelar, assinar, cofre —, primeiros acessos, telas públicas): respiro alto, um elemento dominante, margens generosas. Onde o usuário decide ou hesita, o espaço desacelera e acolhe.

Alvos de toque: mínimo **44px** em viewports ≤768px, sem exceção — inclusive dentro de formulários densos.
...
- **Na barra de navegação:** símbolo a **36px**, à esquerda, com o nome ao lado — âncora do trilho de leitura, com o centro óptico alinhado à linha do cabeçalho. No modo recolhido (64px), símbolo sozinho, centralizado.
...
- **Ação nomeia o efeito** ("Desmarcar consulta" → toast "Consulta desmarcada", com "Desfazer" quando reversível).
...
**Sempre:** leitura contínua em tinta sobre papel calmo (≥7:1) · gradiente do logo só como gesto de marca e como a costura da moldura · terracota para calor (chip, destaque; texto de destaque apenas no tema escuro) · estado com ícone ou rótulo · fonte mono com números tabulares em coluna de dados · fontes empacotadas com a aplicação · alvo de toque ≥44px no mobile · entrada de superfície só com opacidade.
