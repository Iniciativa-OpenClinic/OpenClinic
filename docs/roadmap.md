...
A API é **REST** — o estilo de conversa da web, o mesmo do FHIR e do RNDS; o **contrato OpenAPI** a descreve, e o **FHIR** dá a forma do dado clínico que ela carrega. O mapeamento dos endpoints nasce nesta fase, derivado de [`modulos.md`](./modulos.md) e [`cadastros.md`](./cadastros.md) — é a decisão [0004](./decisions/0004-api-antes-de-interface.md) em forma de artefato. **Se o contrato é escrito antes do código ou gerado a partir dele é decisão em aberto** [0008](./decisions/0008-contrato-antes-ou-depois-do-codigo.md), do conselho fundador; nas duas hipóteses ele descreve a API por inteiro e é revisado como interface pública, não como subproduto. O desenho da API é neutro de linguagem e serve de insumo à decisão [0005](./decisions/0005-linguagem-do-backend.md), ainda em aberto — o tamanho e a forma reais da API pesam nela. O contrato aplica **minimização por endpoint** — cada um devolve o mínimo da sua finalidade — e coleções são **paginadas por padrão**. O desenho do contrato abre uma decisão de arquitetura a registrar em [`decisions/`](./decisions/): o formato dos endpoints — API no padrão FHIR puro, ou API própria com os dados clínicos em recursos FHIR e uma fachada FHIR para interoperabilidade.
