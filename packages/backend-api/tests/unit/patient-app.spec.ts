...
const jwtConfig = { secretKey: 'patient-tests-key-with-at-least-thirty-two-characters' };
const apps: Awaited<ReturnType<typeof buildApp>>[] = [];
afterEach(async () => { await Promise.all(apps.splice(0).map(app => app.close())); });

async function fixture(tenantId: string | undefined = 'tenant-a', role: string = UserRole.OWNER) {
  const patients = { list: vi.fn().mockResolvedValue({ items: [], total: 0 }) };
  const uow = {
    ...
    patientsForTenant: vi.fn().mockReturnValue(patients),
  };
...
describe('Patient application integration', () => {
  it('registers all five endpoints in the served OpenAPI document', async () => {
    ...
    for (const [path, methods] of [
      ['/api/v1/business/patients', ['get', 'post']],
      ['/api/v1/business/patients/{id}', ['get', 'put', 'delete']],
    ] as const) {
  ...
  it('requires authentication before accessing the repository', async () => {
    ...
    expect((await app.inject('/api/v1/business/patients')).statusCode).toBe(401);
    expect(uow.patientsForTenant).not.toHaveBeenCalled();
  });

  it('selects the authenticated tenant repository', async () => {
    const { app, uow, headers } = await fixture();
    const response = await app.inject({ url: '/api/v1/business/patients', headers });
    expect(response.statusCode).toBe(200);
    expect(uow.patientsForTenant).toHaveBeenCalledWith('tenant-a');
  });

  it('rejects a user without patient permissions', async () => {
    const { app, uow, headers } = await fixture('tenant-a', UserRole.USER);
    expect((await app.inject({ url: '/api/v1/business/patients', headers })).statusCode).toBe(403);
    expect(uow.patientsForTenant).not.toHaveBeenCalled();
  });
