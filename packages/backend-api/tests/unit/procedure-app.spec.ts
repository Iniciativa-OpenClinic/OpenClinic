...
const jwtConfig = { secretKey: 'procedure-tests-key-with-at-least-thirty-two-characters' };
...
async function fixture(tenantId: string | undefined = 'tenant-a', role: string = UserRole.OWNER) {
  const procedures = { list: vi.fn().mockResolvedValue({ items: [], total: 0 }) };
  const uow = {
    ...
    proceduresForTenant: vi.fn().mockReturnValue(procedures),
  };
...
describe('Procedure application integration', () => {
  it.each([
    ['GET', '', 'READ'], ['POST', '', 'WRITE'],
    ['PUT', '/procedure-1', 'WRITE'], ['DELETE', '/procedure-1', 'DELETE'],
  ...
  it('registers all five endpoints in the served OpenAPI document', async () => {
    ...
    for (const [path, methods] of [
      ['/api/v1/business/procedures', ['get', 'post']],
      ['/api/v1/business/procedures/{id}', ['get', 'put', 'delete']],
    ] as const) {
  ...
  it('requires authentication before accessing the repository', async () => {
    ...
    expect((await app.inject('/api/v1/business/procedures')).statusCode).toBe(401);
    expect(uow.proceduresForTenant).not.toHaveBeenCalled();
  });

  it('selects the authenticated tenant repository', async () => {
    const { app, uow, headers } = await fixture();
    const response = await app.inject({ url: '/api/v1/business/procedures', headers });
    expect(response.statusCode).toBe(200);
    expect(uow.proceduresForTenant).toHaveBeenCalledWith('tenant-a');
  });

  it('rejects a user without procedure permissions', async () => {
    const { app, uow, headers } = await fixture('tenant-a', UserRole.USER);
    expect((await app.inject({ url: '/api/v1/business/procedures', headers })).statusCode).toBe(403);
    expect(uow.proceduresForTenant).not.toHaveBeenCalled();
  });
