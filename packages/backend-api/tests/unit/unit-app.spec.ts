...
const jwtConfig = { secretKey: 'unit-tests-key-with-at-least-thirty-two-characters' };
const apps: Awaited<ReturnType<typeof buildApp>>[] = [];
afterEach(async () => { await Promise.all(apps.splice(0).map(app => app.close())); });

async function fixture(tenantId: string | undefined = 'tenant-a', role: string = UserRole.OWNER) {
  const units = { list: vi.fn().mockResolvedValue({ items: [], total: 0 }) };
  const uow = {
    ...
    unitsForTenant: vi.fn().mockReturnValue(units),
  };
...
describe('Unit application integration', () => {
  it('registers all five endpoints in the served OpenAPI document', async () => {
    ...
    for (const [path, methods] of [
      ['/api/v1/business/units', ['get', 'post']],
      ['/api/v1/business/units/{id}', ['get', 'put', 'delete']],
    ] as const) {
  ...
  it('requires authentication before accessing the repository', async () => {
    ...
    expect((await app.inject('/api/v1/business/units')).statusCode).toBe(401);
    expect(uow.unitsForTenant).not.toHaveBeenCalled();
  });

  it('selects the authenticated tenant repository', async () => {
    const { app, uow, headers } = await fixture();
    const response = await app.inject({ url: '/api/v1/business/units', headers });
    expect(response.statusCode).toBe(200);
    expect(uow.unitsForTenant).toHaveBeenCalledWith('tenant-a');
  });

  it('rejects a user without unit permissions', async () => {
    const { app, uow, headers } = await fixture('tenant-a', UserRole.USER);
    expect((await app.inject({ url: '/api/v1/business/units', headers })).statusCode).toBe(403);
    expect(uow.unitsForTenant).not.toHaveBeenCalled();
  });
