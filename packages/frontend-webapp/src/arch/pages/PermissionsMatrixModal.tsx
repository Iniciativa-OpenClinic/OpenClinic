...
const SIDEBAR_SECTIONS: MenuSectionSpec[] = [
  ...
  // ── 2. ABA SISTEMA / ARQUITETURA (ARCH) ──
  {
    id: 'system',
...
export const PermissionsMatrixModal: React.FC<PermissionsMatrixModalProps> = ({
  ...
  const toggleSectionAll = (sectionItems: MenuItemSpec[]) => {
    ...
    // Verifica se todos os itens e todas as ações já estão concedidos
    ...
    for (const key of allKeys) {
      ...
      if (allGranted) {
        ...
        // Marcar todas as ações para o grupo
  ...
  // Toggle de uma ação específica para todos os itens da seção
  const toggleSectionAction = (sectionItems: MenuItemSpec[], action: ResourceAction) => {
    ...
    for (const key of allKeys) {
      ...
      if (allHaveAction) {
        // Remover ação (se herdado por usuário, bloqueia com DENY; se direto, remove ALLOW)
        if (permissionTarget.type === PermissionTargetType.USER && isInherited) {
          if (allowIndex >= 0) {
            const p = { ...perms[allowIndex] };
            p.actions = p.actions.filter((a) => a !== action);
        ...
        // Conceder ação (se tinha DENY, remove DENY; se não herdado, adiciona ALLOW)
        if (denyIndex >= 0) {
          const p = { ...perms[denyIndex] };
          p.actions = p.actions.filter((a) => a !== action);
          if (p.actions.length === 0) perms.splice(denyIndex, 1);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Tabela de Permissões com Check por Grupo */}
                  ...
                  {currentSections.map((section) => {
                    return (
                      <React.Fragment key={section.id}>
                        {/* Linha Cabeçalho do Grupo de Menu */}
                        <tr style={{ background: '#f1f5f9', borderTop: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0' }}>
                          ...
                          {/* Checkboxes por ação na linha do grupo */}
